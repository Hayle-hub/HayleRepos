 ...
 *  The DogGenetics class implements a class which prompts the user for their
 * dog's name then randomly generates a breed profile of percentages and
 * corresponding breed. All of which is printed to the console.
 */
package com.hg.doggenetics;
...
public class DogGenetics {
    ...
    public static String[] getBreeds(int numBreeds) {
        ...
        /*
        Generates a non-repeating set of random integers in the range of 1-5.  The first random integer
        ...
        */
        for (int i = 0; i < breeds.length; i++) {
            if (i == 0) {
                ...
                do {
                    repeatValue = false;
                    breeds[i] = Integer.toString(randomNum.nextInt(5) + 1);// shifting 
                    for (int j = 0; j <= i - 1; j++) {
        ...
        for (int i = 0; i < breeds.length; i++) {
            switch (breeds[i]) {
                ...
                case "2":
                    ...
                    break;
                case "3":
                    breeds[i] = "Dramatic RedNosed Asian Pug";
                    break;
                case "4":
                    breeds[i] = "Common Cur";
                    break;
                case "5":
                    breeds[i] = " King Doberman";
                    break;
